package psd_contigs::psd_contigsClient;
...
sub new
{
    ...
    my $self = {
	client => psd_contigs::psd_contigsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a psd_contigs.workspace_name
$contigset_id is a psd_contigs.contigset_id
$return is a psd_contigs.CountContigsResults
...
# Authentication: required
	...
	method => "psd_contigs.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for psd_contigs::psd_contigsClient\n";
    }
    if ($sMajor == 0) {
        warn "psd_contigs::psd_contigsClient version is $svr_version. API subject to change.\n";
    }
}
...
package psd_contigs::psd_contigsClient::RpcClient;
