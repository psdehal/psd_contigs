...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'psd_contigs'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from psd_contigs.psd_contigsImpl import psd_contigs
impl_psd_contigs = psd_contigs(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['psd_contigs.count_contigs_async'] = ['psd_contigs', 'count_contigs']
async_check_methods['psd_contigs.count_contigs_check'] = ['psd_contigs', 'count_contigs']
sync_methods['psd_contigs.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'psd_contigs'
        ...
        self.rpc_service.add(impl_psd_contigs.count_contigs,
                             name='psd_contigs.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['psd_contigs.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "psd_contigs but no authentication header was passed"
                            raise err
